...
module.exports = app => {
  ...
  function process() {
    queueClient.process(async function(task) {
      const pathName = _.get(task.data, 'meta.pathName');
      if (!pathName) throw new Error('Missing task pathName meta data.');
      const ctx = app.createAnonymousContext();
      const process = _.get(ctx, `${pathName}.process`);
      if (!process) throw new Error(`The task ${pathName} is not exists.`);
      return process(task);
    });
  }
};
